...
name: setup-ci
description: >
  Wire per-test line coverage from the user's test suite into Kanon so
  ...
  "connect my tests to Kanon".
disable-model-invocation: true
---

# Kanon CI coverage setup

Kanon infers claims from code — they land **Asserted**. A claim becomes
...
collector writes per-worker shards to `.kanon/coverage/` — add that to
`.gitignore`.
...
Auth in CI is `KANON_API_TOKEN` (a workspace machine token) + `KANON_URL`
in the environment — same as the other CI/Make targets. `repoSlug` may be omitted
if `.kanon/config.json` or `KANON_REPO_SLUG` is set.
...
- `node $PLUGIN/cli.js push-tests <repoSlug>` — or the `kanon_push_tests`
...
- **401 / 404** — see `/kanon:setup` (auth) and the push skill (slug
